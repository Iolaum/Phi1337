...
training_data = pd.read_csv("train.csv", encoding="ISO-8859-1") #read train.csv
testing_data = pd.read_csv("test.csv", encoding="ISO-8859-1") #read test.csv
attribute_data = pd.read_csv("attributes.csv") #read attributes.csv
descriptions = pd.read_csv("product_descriptions.csv") #read product_descriptions.csv

training_data = pd.merge(training_data, descriptions, on="product_uid", how="left") #merge data

product_counts = pd.DataFrame(pd.Series(training_data.groupby(["product_uid"]).size(), name="product_count")) #count
...
training_data = pd.merge(training_data, brand_names, on="product_uid", how="left") #merge brand names
